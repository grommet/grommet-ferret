...
const handlers = {
  ...
  [IMAGE_ADD_PROGRESS]: (state, action) => {
    let uploads = state.uploads.slice(0);
  ...
  [IMAGE_ADD_UPLOADED]: (state, action) => {
    let uploads = state.uploads.slice(0);
    // Done, remove it.
    for (let i=0; i<uploads.length; i++) {
      if (uploads[i].file === action.file) {
  ...
  [IMAGE_ADD_SUCCESS]: (state, action) => {
    let uploads = state.uploads.slice(0);
    // Done, remove it.
    for (let i=0; i<uploads.length; i++) {
      if (uploads[i].file === action.file) {
  ...
  [IMAGE_ADD_FAILURE]: (state, action) => {
    let uploads = state.uploads.slice(0);
    // Done, remove it.
    for (let i=0; i<uploads.length; i++) {
      if (uploads[i].file === action.file) {
  ...
  [IMAGE_UPDATE_PROGRESS]: (state, action) => {
    let uploads = state.uploads.slice(0);
    // preserve 50% for the subsequent task of unpacking the file
    const percent = Math.round(action.loaded / action.total * 50);
    for (let i=0; i<uploads.length; i++) {
  ...
  [IMAGE_UPDATE_UPLOADED]: (state, action) => {
    let uploads = state.uploads.slice(0);
    // Done, remove it.
    for (let i=0; i<uploads.length; i++) {
      if (uploads[i].file === action.file) {
  ...
  [IMAGE_UPDATE_SUCCESS]: (state, action) => {
    let uploads = state.uploads.slice(0);
    // Done, remove it.
    for (let i=0; i<uploads.length; i++) {
      if (uploads[i].file === action.file) {
  ...
  // TODO: Would be good to do something more helpful with this.
  [IMAGE_UPDATE_FAILURE]: (state, action) => {
    let uploads = state.uploads.slice(0);
    // Done, remove it.
    for (let i=0; i<uploads.length; i++) {
      if (uploads[i].file === action.file) {
